...
import { EditorProvider, useCurrentEditor, EditorContent } from '@tiptap/react'; // Added EditorContent import
    ...
    if (editor) {
      ...
      try {
        const response = await fetch('http://localhost:8000/api/document/', {
          ...
          body: JSON.stringify({ content: documentData, template: { name: 'YourTemplateName' } }) // Adjust accordingly
        });
        if (response.ok) {
